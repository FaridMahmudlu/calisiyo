...
export function isShopierProductActive(product) {
  const status = String(product?.stockStatus || product?.status || '')
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '');
  return ['instock', 'active', 'available', 'onsale', 'published'].includes(status);
}
