...
import { isExactProducerDiscount } from '@/lib/billing/content-producer.mjs';

const ORDER_FIELDS = [
  'id', 'user_id', 'plan_code', 'billing_period', 'amount', 'list_amount', 'paid_amount', 'verified_discount_amount', 'currency', 'status',
  'payment_provider', 'customer_email_snapshot', 'expected_provider_product_id',
  'provider_order_id', 'provider_checkout_url', 'provider_status', 'provider_discount_id', 'created_at',
  'creator_signup_attribution_id', 'pricing_source', 'expected_paid_amount', 'expected_discount_amount',
].join(',');
...
async function findCandidateOrders(admin, normalized, product) {
  ...
  let query = admin
    ...
    .eq('pricing_source', product.pricingSource)
...
async function verifiedDiscountBinding(admin, normalized) {
  if (!normalized.discountMinor) return { id: null, method: null };
  if (normalized.discounts.length !== 1 || normalized.discounts[0].method !== 'discountCode') return null;
  const discount = normalized.discounts[0];
  if (!discount.id) return null;
  const { data: binding, error } = await admin
    .from('content_producer_code_bindings')
    .select('code_id,provider_discount_id,status,valid_from,valid_to,configuration')
    .eq('provider', 'shopier')
    .eq('provider_discount_id', discount.id)
    .maybeSingle();
  if (error) throw error;
  if (!binding || binding.configuration?.productScopeConfirmed !== true) return null;
  const { data: code, error: codeError } = await admin
    .from('content_producer_codes')
    .select('discount_bps')
    .eq('id', binding.code_id)
    .maybeSingle();
  if (codeError) throw codeError;
  if (!code || Number(code.discount_bps) !== 2000) return null;
  const paidAt = Date.parse(normalized.dateCreated);
  const starts = Date.parse(binding.valid_from);
  const ends = binding.valid_to ? Date.parse(binding.valid_to) : Number.POSITIVE_INFINITY;
  if (!Number.isFinite(paidAt) || !Number.isFinite(starts) || paidAt < starts || paidAt >= ends) return null;
  return { id: discount.id, method: discount.method };
}
...
export async function verifyAndApplyShopierOrder(providerOrderId, { client, admin } = {}) {
  ...
  if (internalOrder.pricing_source === 'signup_creator_code' && normalized.discountMinor > 0) {
    await markOrdersForReview(db, [internalOrder], normalized.id, normalized.paymentStatus, 'creator_discount_stacking', { attachProviderOrder: true });
    return { outcome: 'review_required', orderId: internalOrder.id, providerOrderId: normalized.id, reason: 'creator_discount_stacking' };
  }
  const discountBinding = await verifiedDiscountBinding(db, normalized);
  if (normalized.discountMinor > 0 && !discountBinding) {
    await markOrdersForReview(db, [internalOrder], normalized.id, normalized.paymentStatus, 'discount_binding_mismatch', { attachProviderOrder: true });
    return { outcome: 'review_required', orderId: internalOrder.id, providerOrderId: normalized.id, reason: 'discount_binding_mismatch' };
  }
  if (normalized.discountMinor > 0 && !isExactProducerDiscount({
    listMinor: normalized.subtotalMinor,
    paidMinor: normalized.totalMinor,
    discountMinor: normalized.discountMinor,
  })) {
    await markOrdersForReview(db, [internalOrder], normalized.id, normalized.paymentStatus, 'discount_amount_mismatch', { attachProviderOrder: true });
    return { outcome: 'review_required', orderId: internalOrder.id, providerOrderId: normalized.id, reason: 'discount_amount_mismatch' };
  }
  const listMinor = Number(normalized.subtotalMinor);
  const paidMinor = Number(normalized.totalMinor);
  const providerExpectedAmount = internalOrder.pricing_source === 'signup_creator_code'
    ? internalOrder.expected_paid_amount
    : (Number.isSafeInteger(paidMinor) ? (paidMinor / 100).toFixed(2) : null);
  const providerExpectedList = internalOrder.pricing_source === 'signup_creator_code'
    ? internalOrder.expected_paid_amount
    : (internalOrder.list_amount || internalOrder.amount);
  const validation = validateShopierOrder(authoritative, {
    productId: internalOrder.expected_provider_product_id,
    listAmount: providerExpectedList,
    amount: providerExpectedAmount,
    providerDiscountId: discountBinding?.id || null,
  ...
  const providerPayload = {
    ...
    listAmount: (listMinor / 100).toFixed(2),
    amount: (validation.order.totalMinor / 100).toFixed(2),
    discountAmount: (validation.order.discountMinor / 100).toFixed(2),
    providerDiscountId: discountBinding?.id || null,
    discountMethod: discountBinding?.method || null,
    currency: normalized.currency,
    quantity: validation.order.items[0].quantity,
    customerEmail: normalized.email,
    dateCreated: normalized.dateCreated,
    paidAt: normalized.dateCreated,
  };
  ...
  if (error) {
    if (isSafeReviewDatabaseError(error)) {
      await markOrdersForReview(
        db, [internalOrder], normalized.id, normalized.paymentStatus,
        'provider_confirmation_conflict', { attachProviderOrder: true },
      );
      return {
        outcome: 'review_required', orderId: internalOrder.id,
...
export async function verifyAndApplyShopierRefund(providerRefundId, { client, admin } = {}) {
  ...
  const validation = validateShopierRefund(authoritative, { amount: internalOrder.paid_amount || internalOrder.amount });
  if (!validation.ok) {
    await markOrdersForReview(
      db,
      [internalOrder],
