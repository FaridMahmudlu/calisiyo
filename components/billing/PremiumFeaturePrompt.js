...
export default function PremiumFeaturePrompt({ open, onClose, feature, requiredPlan = 'Odak', currentPlan = 'Başlangıç', description, benefits = [] }) {
  return (
    <Modal open={open} onClose={onClose} title={`${feature || 'Premium özellik'} · Premium`} description="Bu özellik planına göre açılır veya daha geniş limitlerle kullanılabilir.">
      <div className="premium-prompt">
        ...
        <div className="premium-prompt-copy"><span><Sparkles size={14} /> {requiredPlan} planı ve üzeri</span><h3>{feature} için çalışma alanını büyüt.</h3><p>{description}</p></div>
        ...
        <div className="premium-prompt-actions"><button type="button" onClick={onClose}>Şimdi değil</button><Link href="/dashboard/abonelik">Paketleri karşılaştır <ArrowRight size={15} /></Link></div>
      </div>
    </Modal>
  );
}
