...
export default function BillingPage() {
  ...
  return <div className="billing-page">
    ...
    <section className="billing-orders">
      ...
      {billing?.orders?.length ? <div className="billing-order-list">{billing.orders.map((order) => { const meta = STATUS[order.status] || ['Bilinmiyor', 'muted']; const plan = getPublicPlan(order.plan_code); const paid = order.paid_amount ?? order.amount; const discounted = Number(order.verified_discount_amount || 0) > 0; return <article key={order.id}><div><strong>{order.order_number}</strong><span>{plan.name} · {plan.duration || order.billing_period}</span><time>{new Intl.DateTimeFormat('tr-TR', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(order.created_at))}</time></div><strong>{formatTry(paid)}{discounted && <small>{formatTry(order.list_amount || order.amount)} liste · {formatTry(order.verified_discount_amount)} indirim</small>}</strong><span className={`order-status is-${meta[1]}`}>{meta[0]}</span>{['payment_link_ready', 'awaiting_review'].includes(order.status) && <div className="order-actions">{order.paymentUrl && <a href={order.paymentUrl} target="_blank" rel="noopener noreferrer">Shopier ödeme sayfası <ExternalLink /></a>}<button disabled={busy === order.id} onClick={() => verifyOrder(order.id)}><ShieldCheck /> Ödemeyi doğrula</button></div>}</article>; })}</div> : <div className="billing-empty"><ReceiptText /><strong>Henüz ücretli siparişin yok</strong><p>Satın alma açıldığında sipariş ve ödeme durumu burada görünür.</p></div>}
    </section>
