...
const FILTERS = [
  ...
  ['approved', 'Onaylandı'], ['refunded', 'İade edildi'], ['cancelled', 'İptal edildi'],
  ['rejected', 'Reddedildi'], [null, 'Tümü'],
];
...
export default function AdminPaymentsPage() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    setLoading(false);
    if (error || eventError) return setNotice({ type: 'error', message: (error || eventError).message });
    setOrders(data || []);
  ...
  const review = async (decision) => {
    if (!selected) return;
    if (decision === 'approve' && reference.trim().length < 4) return setNotice({ type: 'error', message: 'Shopier panelindeki benzersiz ödeme referansını gir.' });
    setBusy(decision);
  ...
  return <div className="admin-dashboard admin-payments-page">
    ...
    <section className="admin-page-heading"><div><span><CreditCard size={15} /> Ödeme yönetimi</span><h1>Shopier siparişleri</h1><p>Otomatik eşleşmeyen ödemeleri Shopier kanıtlarıyla güvenli biçimde incele.</p></div><div><button className="admin-refresh" onClick={loadOrders}><RefreshCw size={15} /> Yenile</button></div></section>
    ...
    <section className="admin-card admin-payment-queue">
      <header><div><span>Provider olayları</span><h2>Manuel eşleştirme gerektiren Shopier bildirimleri</h2></div></header>
      {providerEvents.length ? <div className="admin-table-wrap"><table><thead><tr><th>Olay</th><th>Shopier siparişi</th><th>Neden</th><th>Deneme</th><th>Tarih</th></tr></thead><tbody>{providerEvents.map((event) => <tr key={event.id}><td><strong>{event.eventType}</strong><small>{event.eventId}</small></td><td>{event.providerOrderId || '—'}</td><td>{event.reasonCode || event.lastErrorCode || 'İnceleme gerekli'}</td><td>{event.attempts}</td><td>{formatDate(event.updatedAt || event.createdAt)}</td></tr>)}</tbody></table></div> : <div className="admin-empty"><CheckCircle2 /><strong>Bekleyen provider olayı yok</strong><span>Otomatik eşleşmeyen Shopier bildirimleri burada görünür.</span></div>}
    </section>
    {selected && <div className="admin-payment-modal" onMouseDown={() => !busy && setSelected(null)}><section role="dialog" aria-modal="true" aria-labelledby="payment-review-title" onMouseDown={(event) => event.stopPropagation()}><header><div><span>Güvenli inceleme</span><h2 id="payment-review-title">{selected.orderNumber}</h2></div><button onClick={() => setSelected(null)} aria-label="Pencereyi kapat">×</button></header><div className="payment-review-summary"><span>Kullanıcı<strong>{selected.fullName || selected.email}</strong></span><span>Paket<strong>{selected.planName} · {periodLabel(selected.billingPeriod)}</strong></span><span>Tutar<strong>{formatMoney(selected.amount, selected.currency)}</strong></span><span>Shopier siparişi<strong>{selected.providerOrderId || 'Henüz eşleşmedi'}</strong></span><span>Ürün kimliği<strong>{selected.providerProductId || '—'}</strong></span><span>Provider durumu<strong>{selected.providerStatus || '—'}</strong></span></div>{selected.paymentUrl && <a className="payment-provider-link" href={selected.paymentUrl} target="_blank" rel="noopener noreferrer">Shopier bağlantısını aç <ExternalLink size={14} /></a>}<label><span>Shopier ödeme referansı</span><input value={reference} onChange={(event) => setReference(event.target.value)} placeholder="Shopier panelindeki benzersiz referans" disabled={selected.status !== 'awaiting_review'} /></label><label><span>İnceleme notu</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Tutar, kullanıcı, ürün ve sipariş eşleşmesi…" disabled={selected.status !== 'awaiting_review'} /></label>{selected.status === 'awaiting_review' && <div className="payment-review-actions"><button className="is-reject" disabled={busy} onClick={() => review('reject')}>{busy === 'reject' ? <LoaderCircle className="is-spinning" /> : <XCircle />} Reddet</button><button className="is-approve" disabled={busy || reference.trim().length < 4} onClick={() => review('approve')}>{busy === 'approve' ? <LoaderCircle className="is-spinning" /> : <CheckCircle2 />} Onayla ve etkinleştir</button></div>}<p className="payment-review-warning">Onay vermeden önce Shopier panelinde tutarı, ürün kimliğini, müşteri hesabını ve ödeme durumunu doğrula. Bu işlem kullanıcıya ücretli erişim verir ve denetim kaydına yazılır.</p></section></div>}
  </div>;
}
