...
export default function AdminPaymentsPage() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    setLoading(false);
    if (error) { setNotice({ type: 'error', message: error.message }); return; }
    setOrders(data || []);
  }, [filter, supabase]);
  ...
  const review = async (decision) => {
    if (!selected) return;
    if (decision === 'approve' && reference.trim().length < 4) {
      setNotice({ type: 'error', message: 'İyzico panelindeki benzersiz ödeme referansını gir.' });
      return;
    }
    setBusy(decision);
  ...
  return <div className="admin-dashboard admin-payments-page">
    {notice && <div className={`admin-notice is-${notice.type}`}>{notice.type === 'success' ? <CheckCircle2 size={17} /> : <XCircle size={17} />}{notice.message}</div>}
    <section className="admin-page-heading"><div><span><CreditCard size={15} /> Ödeme yönetimi</span><h1>İyzico siparişleri</h1><p>Otomatik doğrulanmayan ödemeleri İyzico panelindeki benzersiz referansla güvenli biçimde incele.</p></div><div><button className="admin-refresh" onClick={loadOrders}><RefreshCw size={15} /> Yenile</button></div></section>
    <section className="admin-card admin-payment-queue">
      ...
      {loading ? <div className="admin-detail-loading"><LoaderCircle className="is-spinning" /> Siparişler yükleniyor…</div> : orders.length ? <div className="admin-table-wrap"><table><thead><tr><th>Sipariş / kullanıcı</th><th>Paket</th><th>Tutar</th><th>Durum</th><th>Tarih</th><th>İşlem</th></tr></thead><tbody>{orders.map((order) => <tr key={order.id}><td><strong>{order.orderNumber}</strong><small>{order.fullName || 'Öğrenci'} · {order.email}</small></td><td>{order.planName}<small>{order.billingPeriod === 'annual' ? '365 gün' : '30 gün'}</small></td><td><strong>{formatMoney(order.amount, order.currency)}</strong></td><td><span className={`payment-state is-${order.status}`}>{order.status}</span></td><td>{formatDate(order.claimedAt || order.createdAt)}</td><td><button onClick={() => { setSelected(order); setReference(order.paymentReference || ''); setNote(order.decisionNote || ''); }}>{order.status === 'awaiting_review' ? 'İncele' : 'Detay'}</button></td></tr>)}</tbody></table></div> : <div className="admin-empty"><SearchCheck /><strong>Bu durumda sipariş yok</strong><span>Filtreyi değiştirebilir veya kuyruğu yenileyebilirsin.</span></div>}
    </section>
    {selected && <div className="admin-payment-modal" onMouseDown={() => !busy && setSelected(null)}><section onMouseDown={(event) => event.stopPropagation()}><header><div><span>Güvenli inceleme</span><h2>{selected.orderNumber}</h2></div><button onClick={() => setSelected(null)}>×</button></header><div className="payment-review-summary"><span>Kullanıcı<strong>{selected.fullName || selected.email}</strong></span><span>Paket<strong>{selected.planName} · {selected.billingPeriod === 'annual' ? '365 gün' : '30 gün'}</strong></span><span>Tutar<strong>{formatMoney(selected.amount, selected.currency)}</strong></span></div>{selected.paymentUrl && <a className="payment-provider-link" href={selected.paymentUrl} target="_blank" rel="noopener noreferrer">İyzico bağlantısını aç <ExternalLink size={14} /></a>}<label><span>İyzico ödeme referansı</span><input value={reference} onChange={(event) => setReference(event.target.value)} placeholder="İyzico panelindeki benzersiz referans" disabled={selected.status !== 'awaiting_review'} /></label><label><span>İnceleme notu</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Tutar, kullanıcı ve sipariş eşleşmesi…" disabled={selected.status !== 'awaiting_review'} /></label>{selected.status === 'awaiting_review' && <div className="payment-review-actions"><button className="is-reject" disabled={busy} onClick={() => review('reject')}>{busy === 'reject' ? <LoaderCircle className="is-spinning" /> : <XCircle />} Reddet</button><button className="is-approve" disabled={busy || reference.trim().length < 4} onClick={() => review('approve')}>{busy === 'approve' ? <LoaderCircle className="is-spinning" /> : <CheckCircle2 />} Onayla ve etkinleştir</button></div>}<p className="payment-review-warning">Onay vermeden önce İyzico panelinde tutar, sipariş ve ödeme durumunu doğrula. Bu işlem kullanıcıya ücretli erişim verir ve denetim kaydına yazılır.</p></section></div>}
  </div>;
}
